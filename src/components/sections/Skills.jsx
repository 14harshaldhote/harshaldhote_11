...
const CategoryCard = ({ category, isActive, isVisible }) => {
  return (
    <div className={`p-8 transition-all duration-500 ${isActive ? 'opacity-100 max-h-screen' : 'opacity-0 max-h-0 overflow-hidden'}`}>
      <div className="mb-8 flex items-start">
        <div className={`text-5xl mr-4 transition-all duration-500 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 -translate-y-4'}`}>
          {category.icon}
        </div>
        <div className={`transition-all duration-500 delay-100 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}>
          <h3 className="text-2xl font-bold text-[var(--text-heading)]">{category.category}</h3>
          <p className="text-base text-[var(--text-primary)] opacity-80 mt-1 max-w-2xl">{category.description}</p>
        </div>
      </div>
  ...
  return (
    ...
    >
      ...
      <div className="absolute top-0 left-0 w-full h-full overflow-hidden opacity-10 pointer-events-none">
        <div className="absolute top-10 left-10 w-72 h-72 rounded-full bg-[var(--accent-primary)] blur-3xl opacity-10"></div>
        <div className="absolute bottom-20 right-20 w-96 h-96 rounded-full bg-[var(--accent-primary)] blur-3xl opacity-10"></div>
      </div>
      ...
      <div className="container mx-auto max-w-6xl px-4 sm:px-6 lg:px-8 relative z-10">
        <div className={`transition-all duration-1000 ${isInView ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
          ...
          <p className="text-center text-[var(--text-primary)] text-lg max-w-3xl mx-auto mt-6 opacity-80">
            ...
            <span className="block text-sm mt-2">Explore each category to see my technical experience</span>
          </p>
        </div>
        
        {/* Category Filter Tabs */}
        <div className={`mt-12 flex justify-center transition-all duration-1000 delay-200 ${isInView ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
        ...
        {/* Call to Action */}
        <div className={`mt-24 text-center transition-all duration-1000 delay-500 ${isInView ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
          <div className="relative inline-block group">
            ...
            <p className="text-[var(--text-heading)] text-lg font-medium mb-4 p-4">Interested in how I can apply these skills to your project?</p>
            <a 
              href="mailto:dhoteharshal16@gmail.com"
              className="inline-block px-8 py-4 bg-[var(--accent-primary)] text-[var(--bg-primary)] rounded-lg font-medium text-base shadow-lg transform transition-all duration-300 hover:scale-105 hover:shadow-xl border border-transparent hover:border-[var(--button-border-hover)]"
            >
